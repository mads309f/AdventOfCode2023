...
let AtStartOrEnd x y =
    (x = 0 && y = -1) || (x = len_x-1 && y = len_y)

let IsDead x y time = 
    if AtStartOrEnd x y then
        false
    // out of bounds
    elif (x < 0 || x >= len_x || y < 0 || y >= len_y) then
        true
    else
        ...
        // List.exists (fun b -> let (tx, ty) = BlizzardPos b time in tx = x && ty = y) blizzards
...
let IsFinished x y timeState =
    match timeState with
    | First | Third -> x = len_x - 1 && y = len_y
    | Second -> x = 0 && y = -1
...
while queue.Count > 0 do
    ...
    if dp.Contains (time % lcm_len, x, y) then
        ...
        if IsFinished x y timeState then
            printfn "Rechead at: %A" time
            // reset
            queue.Clear()
            dp.Clear()
